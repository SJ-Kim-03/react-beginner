import { Label } from "../../../components/ui/label";
import { Button } from "../../../components/ui/button";
import { ArrowLeft, Asterisk, BookOpenCheck, ImageOff, Save } from "lucide-react";
import { Input } from "../../../components/ui/input";
import { Select, SelectGroup, SelectTrigger, SelectItem, SelectValue, SelectContent, SelectLabel } from "@/components/ui/select";
import { TOPIC_CATEGORY } from "@/constants/category.constant";
import { AppEditor, AppThumbnailUpload } from "@/components/common";
import { createClient } from "@/lib/supabase/client";
import { useEffect, useState } from "react";
import { toast } from "sonner";
import { useAuthStore } from "@/stores";
// import { nanoid } from "nanoid";
import { useNavigate, useParams } from "react-router";
import type { Block } from "@blocknote/core";
import { TOPIC_STATUS } from "@/types/topic.type";

export default function CreateTopic() {
    const user = useAuthStore((state) => state.user);
    const { id } = useParams(); // URL에서 topic_id 추출
    const [title, setTitle] = useState<string>("");
    const [content, setContent] = useState<Block[]>([]);
    const [category, setCategory] = useState<string>("");
    const [thumbnail, setThumbnail] = useState<File | string | null>(null);
    const navigate = useNavigate();

    useEffect(() => {
        fetchTopic();
    }, []);

    const fetchTopic = async () => {
        const supabase = createClient();
        try {
            const { data: topic, error } = await supabase
                .from("topic")
                .select("*")
                .eq("id", id)

            if (error) {
                toast.error(error.message);
                return;
            }

            if (topic) {
                setTitle(topic[0].title ?? "");
                setContent(topic[0].content ? JSON.parse(topic[0].content) : []);
                setCategory(topic[0].category ?? "");
                setThumbnail(topic[0].thumbnail);
            }
        }
        catch (error) {
            console.log(error);
            throw error;
        }
    }

    const handleSave = async () => {
        const supabase = createClient();
        if (!title || !content || !category || !thumbnail) {
            toast.warning("제목, 본문, 카테고리, 썸네일을 기입하세요.");
            return;
        }
        // 1. 파일 업로드 시, Supabase Storage bucket에 업로드 후, bucket URL을 topic 테이블에 thumbnail 컬럼에 string 타입으로 저장
        let thumbnailUrl: string | null = null;

        if (thumbnail && thumbnail instanceof File) {
            // storage에 업로드
            const fileExt = thumbnail.name.split(".").pop(); // 파일 확장자 추출 pop: 배열에서 마지막 요소 반환
            const fileName = `${id}.${fileExt}`; // 고유한 파일 이름 생성
            const filePath = `topics/${fileName}`; // 파일 경로 설정

            const { error: uploadError } = await supabase.storage
                .from("files")
                .upload(filePath, thumbnail, { upsert: true }); // 같은 이름의 파일이 있을 경우 덮어쓰기

            if (uploadError) throw uploadError;

            // 업로드된 파일의 public URL 가져오기
            const { data } = supabase.storage
                .from("files")
                .getPublicUrl(filePath);

            if (!data) throw new Error("썸네일 Public URL을 가져오는데 실패했습니다.");
            thumbnailUrl = data.publicUrl;
        } else if (typeof thumbnail === "string") {
            // 기존 이미지 유지
            thumbnailUrl = thumbnail;
        }

        const { data, error } = await supabase
            .from('topic')
            // 축약 가능 title: title과 동일
            .update({
                title,
                content: JSON.stringify(content),
                category,
                thumbnail: thumbnailUrl,
                author: user?.id,
                status: TOPIC_STATUS.TEMP, // 저장은 임시 저장으로
            })
            .eq("id", id)
            .select()

        if (error) {
            toast.error(error.message);
            return
        }

        if (data) {
            toast.success("토픽을 저장하였습니다.")
            return
        }
    };

    const handlePublish = async () => {
        const supabase = createClient();
        if (!title || !content || !category || !thumbnail) {
            toast.warning("제목, 본문, 카테고리, 썸네일을 기입하세요.");
            return;
        }
        // 1. 파일 업로드 시, Supabase Storage bucket에 업로드 후, bucket URL을 topic 테이블에 thumbnail 컬럼에 string 타입으로 저장
        let thumbnailUrl: string | null = null;

        if (thumbnail && thumbnail instanceof File) {
            // storage에 업로드
            const fileExt = thumbnail.name.split(".").pop(); // 파일 확장자 추출 pop: 배열에서 마지막 요소 반환
            const fileName = `${id}.${fileExt}`; // 고유한 파일 이름 생성
            const filePath = `topics/${fileName}`; // 파일 경로 설정

            const { error: uploadError } = await supabase.storage
                .from("files")
                .upload(filePath, thumbnail, { upsert: true }); // 같은 이름의 파일이 있을 경우 덮어쓰기

            if (uploadError) throw uploadError;

            // 업로드된 파일의 public URL 가져오기
            const { data } = supabase.storage
                .from("files")
                .getPublicUrl(filePath);

            if (!data) throw new Error("썸네일 Public URL을 가져오는데 실패했습니다.");
            thumbnailUrl = data.publicUrl;
        } else if (typeof thumbnail === "string") {
            // 기존 이미지 유지
            thumbnailUrl = thumbnail;
        }

        const { data, error } = await supabase
            .from('topic')
            // 축약 가능 title: title과 동일
            .update({
                title,
                content: JSON.stringify(content),
                category,
                thumbnail: thumbnailUrl,
                author: user?.id,
                status: TOPIC_STATUS.PUBLISHED,
            })
            .eq("id", id)
            .select()

        if (error) {
            toast.error(error.message);
            return;
        }

        if (data) {
            toast.success("토픽을 발행하였습니다.")
            navigate("/"); // 홈으로

            return
        }
    };

    return <main className="w-full h-full min-h-[1024px] flex gap-6 p-6">
        <div className="fixed right-1/2 bottom-10 translate-x-1/2 z-20 flex items-center gap-2">
            <Button variant={"outline"} size="icon" onClick={() => navigate("/")}>
                <ArrowLeft />
            </Button>
            <Button type="button" variant={"outline"} size="icon" className="w-22 !bg-yellow-800/50 font-stretch-100%" onClick={handleSave}>
                <Save />
                저장
            </Button>
            <Button variant={"outline"} size="icon" className="w-22 !bg-emerald-800/50" onClick={handlePublish}>
                <BookOpenCheck />
                발행
            </Button>
        </div>
        {/* 토픽 작성하기 */}
        <section className="w-3/4 h-full flex flex-col gap-6">
            <div className="flex flex-col pb-6 border-b">
                <span className="text-[#F96859] font-semibold">Step 01</span>
                <span className="text-base font-semibold">토픽 작성하기</span>
            </div>
            <div className="flex flex-col gap-2">
                <div className="flex items-center gap-1">
                    <Asterisk size={14} className="text-[#F96859]" />
                    <Label className="text-muted-foreground">제목</Label>
                </div>
                <Input placeholder="제목을 입력하세요." value={title} onChange={(event) => setTitle(event.target.value)} className="h-16 pl-6 !text-lg placeholder:font-semibold border-0" />
            </div>
            <div className="flex flex-col gap-2">
                <div className="flex items-center gap-1">
                    <Asterisk size={14} className="text-[#F96859]" />
                    <Label className="text-muted-foreground">본문</Label>
                </div>
                {/* BlockNote Text Editor UI */}
                <AppEditor props={content} setContent={setContent} />
            </div>

        </section>
        {/* 카테고리 및 썸네일 등록 */}
        <section className="w-1/4 h-full flex flex-col gap-6">
            <div className="flex flex-col pb-6 border-b">
                <span className="text-[#F96859] font-semibold">Step 02</span>
                <span className="text-base font-semibold">카테고리 및 썸네일 등록</span>
            </div>
            <div className="flex flex-col gap-2">
                <div className="flex items-center gap-1">
                    <Asterisk size={14} className="text-[#F96859]" />
                    <Label className="text-muted-foreground">카테고리</Label>
                </div>
                <Select value={category} onValueChange={(value) => setCategory(value as string)}>
                    <SelectTrigger className="w-full">
                        <SelectValue placeholder="토픽(주제) 선택">
                            {TOPIC_CATEGORY.find((item) => item.category === category)?.label ?? "토픽(주제) 선택"}

                        </SelectValue>
                    </SelectTrigger>
                    <SelectContent>
                        <SelectGroup>
                            <SelectLabel>카테고리(주제)</SelectLabel>
                            {TOPIC_CATEGORY.map((item) => {
                                return (
                                    <SelectItem key={item.id} value={item.category}>
                                        {item.label}
                                    </SelectItem>
                                )
                            })}
                        </SelectGroup>
                    </SelectContent>
                </Select>
            </div>
            <div className="flex flex-col gap-2">
                <div className="flex items-center gap-1">
                    <Asterisk size={14} className="text-[#F96859]" />
                    <Label className="text-muted-foreground">썸네일</Label>
                </div>
                <AppThumbnailUpload file={thumbnail} onChange={setThumbnail} />
                {/* 썸네일 UI */}
                <Button variant="outline" className="border-0" onClick={() => setThumbnail(null)}>
                    <ImageOff />
                    썸네일 제거
                </Button>
            </div>
        </section>
    </main >
}
